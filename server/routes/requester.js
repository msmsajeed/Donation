...
requester.post('/updaterequester',async (req,res) => {
    ...
    if(!username || !newage || !newbloodgroup || !neworganlist || !newpositivedate || !newphone || !newstate || !newcity)
    {
        return res.status(422).json({error:"Please fill all the fileds carefully"});
    }
    try{
...
requester.post('/createrequester',async (req,res)=>{
    ...
    if(!name || !username || !password || !gender || !age || !bloodgroup || !organlist || !positivedate || !phone || !state || !city)
    {
        return res.status(422).json({error:"Please fill all the fileds carefully"});
    }
