...
donor.post('/updatedonor',async (req,res) => {
    ...
    const neworganlist=req.body.organlist;
    ...
    if(!username || !newage || !newbloodgroup || !neworganlist || !newpositivedate || !newnegativedate || !newphone || !newstate || !newcity)
    {
        return res.status(422).json({error:"Please fill all the fileds carefully"});
    }
    try{
        ...
        if(usernameExist)
        {
            ...
            usernameExist.organlist=neworganlist;
...
donor.post('/createdonor',async(req,res)=>{
    ...
    const organlist=req.body.organlist;
    ...
    if(!name || !username || !password || !gender || !age || !bloodgroup || !organlist || !positivedate || !negativedate || !phone || !state || !city)
    {
        return res.status(422).json({error:"Please fill all the fileds carefully"});
    }
